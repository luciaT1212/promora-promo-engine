...
exports.UserUsageLimitRule = void 0;
...
class UserUsageLimitRule extends validation_rule_1.ValidationRule {
    usageRepo;
    ...
    async validate(context) {
        const rule = context.promo?.getRule(promo_code_types_1.RuleType.USER_USAGE_LIMIT);
        if (!rule)
        ...
        const count = await this.usageRepo.countPaidUsesByCodeAndBuyer(context.promo.id, context.buyer.buyerId, excludeIds);
        if (count >= limit) {
...
exports.UserUsageLimitRule = UserUsageLimitRule;
//# sourceMappingURL=user-usage-limit.rule.js.map